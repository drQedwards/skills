...
license	MIT
env	
OPENSEA_API_KEY	PRIVATE_KEY	RPC_URL
description	required	obtain
API key for OpenSea REST API (tool discovery endpoints)
false
https://docs.opensea.io/reference/api-keys#instant-api-key-for-agents
description	required
Wallet private key for onchain registration and tool calls
false
description	required
RPC URL for Base mainnet (default https://mainnet.base.org)
false
dependencies	
node >= 18.0.0
OpenSea Tool SDK
...
POST https://api.opensea.io/api/v2/auth/siwe/nonce with no body and read the single-use { "nonce": "..." } response.
Build and sign this EIP-4361 message. Use the exact canonical statement below as both statement and any client field named originalStatement; do not paraphrase it or add a newline:
...
POST https://api.opensea.io/api/v2/auth/siwe/verify with the parsed message, signature, and { "chainArch": "EVM" }. Verification must bind the recovered signer to message.address, validate the single-use nonce and message fields, and retain the returned session cookies before minting the scoped PAT.
...
Instant key creation is rate limited per IP, so always save a fetched key and reuse it rather than re-fetching. The opensea-api skill ships an auth/opensea-resolve-key.sh helper that does this (env → cached file → fetch + save); see its "API key resolution" section. For higher rate limits, create a full key at Settings → Developer.
...
Fetches the manifest from --metadata URL
Validates the manifest schema
Verifies manifest.creatorAddress matches your wallet
Computes the JCS keccak256 manifest hash
Calls ToolRegistry.registerTool(metadataURI, manifestHash, accessPredicate)
Returns the toolId from the ToolRegistered event
...
PRIVATE_KEY=0x... RPC_URL=https://mainnet.base.org \
  ...
  --body '{"query": "hello"}'
References
references/x402.md: pay-per-call protocol, server-side paywall, paidFetch
references/predicate-gating.md: 402-based predicate access control (zero-value X-Payment), combined gates
references/known-predicates.md: deployed predicate contracts and SDK helpers
Tool SDK GitHub
